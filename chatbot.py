# conda create --name chatWithSql python=3.11

#  conda activate chatWithSql
...
def get_sql_chain(db):
  template = """
    ...
    You present the information in a readable way, and know how to use spaces.
    ...
    Question: What is the best rating in Riyadh branch?
    SQL Query:  SELECT 
                GREATEST(Room_Service, Restaurant, Sanitary_Conditions, Front_Desk, Car_Parking, Amenities) AS best_rating
                FROM 
                customer_rating 
                WHERE 
                branch_id = (SELECT branch_id FROM branches WHERE branch = 'Riyadh branch');
    
                
    Question: what are the ratings for hilton ?
    SQL Query:  SELECT branch, 
                    AVG(`Room_Service`) AS avgRoomService, 
                    AVG(`Restaurant`) AS avgRestaurant, 
                    AVG(`Sanitary_Conditions`) AS avgSanitaryConditions, 
                    AVG(`Front_Desk`) AS avgFrontDesk, 
                    AVG(`Car_Parking`) AS avgCarParking 
                FROM customer_rating 
                INNER JOIN branches ON customer_rating.branch_id = branches.branch_id 
                WHERE branches.company_id = (SELECT company_id FROM companies WHERE company = 'Hilton branch') 
                GROUP BY branch;



    Question: what are the ratings for jeddah ?
    SQL Query:  SELECT branch, 
                    AVG(`Room_Service`) AS avgRoomService, 
                    AVG(`Restaurant`) AS avgRestaurant, 
                    AVG(`Sanitary_Conditions`) AS avgSanitaryConditions, 
                ...
                FROM customer_rating 
                INNER JOIN branches ON customer_rating.branch_id = branches.branch_id 
                WHERE branches.company_id = (SELECT company_id FROM companies WHERE company = 'Jeddah branch') 
                GROUP BY branch;
...
def get_response(user_query: str, db: SQLDatabase, chat_history: list, ratings: dict = None):
    ...
    template = """
    You are a data analyst at a company. You are interacting with a user who is asking you questions about the company's database.
    Based on the table schema below, user question, SQL query, and SQL response, provide a clear and concise natural language answer that directly addresses the user's question.
    ...
    Provide the exact rating value and specify which category or overall rating it corresponds to.
    """
    ...
    query = """
        ...
        FROM customer_rating 
        INNER JOIN branches ON customer_rating.branch_id = branches.branch_id 
        WHERE branches.company_id = (SELECT company_id FROM companies WHERE company = 'Hilton') 
        GROUP BY branch;
    """
  ...
  # Preprocess the user query
    ...
    if any(word in farewells for word in user_words):
